...
class CartController extends \frontend\base\Controller
{
    ...
    public function actionFakePay($orderId)
    {
        $order = Order::findOne([
            'id' => $orderId,
        ...
        $order->status = Order::STATUS_PAID;
        $order->transaction_id = 'FAKE-' . time();
        $order->paypal_order_id = 'FAKEPAY-' . time();
        if ($order->save()) {
            if (!$order->sendEmailToVendor()) {
                Yii::error("Email to the vendor is not sent");
            }
        ...
        throw new \yii\web\ServerErrorHttpException("Could not save order");
    }
